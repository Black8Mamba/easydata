...
> 注：预留区大小等于一个Flash块大小(如2KB)，用于原子写入时的临时存储。
...
事务状态定义见 4.2 节 `kv_tx_state_persist_t`，持久化到Flash头部。

事务流程：
1. `tx_begin()` → 设置状态为 PREPARED
2. 写入数据到预留区
3. `tx_commit()` → 递增版本号，状态恢复 IDLE
4. `tx_rollback()` → 预留区数据作废
...
> 注：哈希表大小可通过 `FLASH_KV_HASH_SIZE` 配置，建议设置为预期最大条目的 1.5-2 倍。
